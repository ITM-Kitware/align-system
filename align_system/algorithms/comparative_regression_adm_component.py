...
class ChoiceRelevanceClassificationComponent(ADMComponent):
    ...
    def run_returns(self):
        return ('attribute_relevance_reasonings',
                'attribute_relevance_binary',
                'attribute_dialogs')
    ...
    def run(self,
        ...
        attribute_dialogs = {}
        attribute_relevance_binary = {}
        attribute_relevance_reasonings = {}
        for attribute in target_attributes:
            scenario_description = call_with_coerced_args(
                self.scenario_description_template,
                {'scenario_state': scenario_state,
            ...
            log.info("[bold]*RELEVANCE CLASSIFICATION DIALOG PROMPT*[/bold]",
                     extra={"markup": True})
            log.info(dialog_prompt)
            ...
            for i, response in enumerate(responses):
                ...
                for choice in choices:
                    attribute_relevance_binary.setdefault(choice, {})
                    attribute_relevance_binary[choice].setdefault(
                        attribute.kdma, []).append(0 if response[choice]['relevant'] == 'no' else 1)

                    attribute_relevance_reasonings.setdefault(choice, {})
                    attribute_relevance_reasonings[choice].setdefault(
                        attribute.kdma, []).append(response[choice]['reasoning'])

            attribute_dialogs[attribute.kdma] = dialog

        return attribute_relevance_reasonings, attribute_relevance_binary, attribute_dialogs

class ProbeRelevanceClassificationComponent(ADMComponent):
    def __init__(self,
                 structured_inference_engine,
                 scenario_description_template,
                 prompt_template,
    ...
    def run(self,
        ...
        for attribute in target_attributes:
            ...
            for i, response in enumerate(responses):
                ...
                # Repeat same reasoning and relevance for each choice
                for choice in choices:
                    attribute_relevance_binary.setdefault(choice, {})
                    attribute_relevance_binary[choice].setdefault(
                        attribute.kdma, []).append(0 if response['relevant'] == 'no' else 1)
