...
from align_system.utils.structured_data_utils import flatten, military_disposition_mapping, rapport_mapping, action_type_mapping
...
class HeuristicADM(ActionBasedADM):
    def __init__(self, misaligned: bool=False, probabilistic: bool=False):
        self.model = joblib.load("knn_io.pkl")
    ...
    def choose_action(self, scenario_state, available_actions, alignment_target,
        ...
        if alignment_target is None:
            raise ValueError("Heuristic ADM needs alignment target")
        ...
        character_dict = defaultdict()
        for character in scenario_state.characters:
            character_dict[character.id] = flatten(character.to_dict())

        # get predicted kdma values
        pred_kdma_values = {}
        actions_with_chars = []
        for action in available_actions:
            if hasattr(action, 'character_id') and action.character_id is not None:
                cid = action.character_id
                num_injuries = len(character_dict[cid]['injuries'])
                # Do pre-processing of features
                feat_vec = [
                    military_disposition_mapping[character_dict[cid]['demographics_military_disposition']],
                    rapport_mapping[character_dict[cid]['rapport']],
        ...
        if all_scalar_targets:
            if distribution_matching == 'average':
                ...
                selected_choice_id, probs = alignment_function(
                    pred_kdma_values, target_kdmas, misaligned=self.misaligned, probabilistic=self.probabilistic
                )
            elif distribution_matching == 'cumulative_average':
                alignment_function = alignment_utils.CumulativeAvgDistScalarAlignment()
                selected_choice_id, probs = alignment_function(
                    pred_kdma_values, target_kdmas, self.choice_history, misaligned=self.misaligned, kde_norm=kde_norm, probabilistic=self.probabilistic
                )
            else:
            ...
            if distribution_matching == 'cumulative_kde':
                ...
                selected_choice_id, probs = alignment_function(
                    pred_kdma_values, target_kdmas, self.choice_history, misaligned=self.misaligned,
                ...
                selected_choice_id, probs = alignment_function(
                    pred_kdma_values, target_kdmas, misaligned=self.misaligned, kde_norm=kde_norm, probabilistic=self.probabilistic
                )
        else:
        ...
        for target_kdma in target_kdmas:
            ...
            self.choice_history[kdma].extend(pred_kdma_values[selected_choice_id][kdma])
        ...
        results = pd.DataFrame([
            ...
            for action in actions_with_chars
