...
class KaleidoSys:
    def __init__(
        self,
        model_name="tsor13/kaleido-small",
    ...
    def load_model(self):
        ...
        self.valence_ids = [
            self.token_ids["Supports"],
            self.token_ids["Opposes"],
            self.token_ids["Either"],
        ]
    ...
    def deduplicate(
        ...
        # dedup with ngrams
        df = self.deduplicate_rouge(
            df,
            rouge_type=rouge_type,
            threshold=ngram_threshold,
            dedup_across_vrd=dedup_across_vrd,
        )
        # dedup with embeddings
        df = self.deduplicate_embed(
            df,
    ...
    def remove_by_relevance(self, df, relevance_threshold=None):
        ...
        if isinstance(relevance_threshold, dict):
            relevance_threshold_value = relevance_threshold.get(
                "Value", self.model.config.system_params["relevance_threshold"]["Value"]
            )
            relevance_threshold_right = relevance_threshold.get(
                "Right", self.model.config.system_params["relevance_threshold"]["Right"]
            )
            relevance_threshold_duty = relevance_threshold.get(
                "Duty", self.model.config.system_params["relevance_threshold"]["Duty"]
            )
            if not set(relevance_threshold.keys()).issubset({"Value", "Right", "Duty"}):
                raise ValueError(
                    "Relevance threshold dictionary can only contain 'Value', 'Right', and 'Duty'"
                )
        else:
            relevance_threshold_value = relevance_threshold_right = (
                relevance_threshold_duty
            ) = relevance_threshold

        before_size = len(df)
        df = df[
            (df["vrd"] == "Value") & (df["relevant"] >= relevance_threshold_value)
            | (df["vrd"] == "Right") & (df["relevant"] >= relevance_threshold_right)
            | (df["vrd"] == "Duty") & (df["relevant"] >= relevance_threshold_duty)
        ]
        after_size = len(df)
        print(
            "Dropping {} rows because of relevance threshold".format(
                before_size - after_size
            )
        )
        return df

    def deduplicate_rouge(
        self, df, rouge_type="rougeL", threshold=None, dedup_across_vrd=False
    ):
        ...
        if isinstance(threshold, dict):
            ...
            if not set(threshold.keys()).issubset({"fmeasure", "precision", "recall"}):
                raise ValueError(
                    "Threshold dictionary can only contain 'fmeasure', 'precision', and 'recall'"
                )
        else:
            threshold = {
                "fmeasure": threshold,
        ...
        for i in range(1, len(df)):
            for j in range(i):
                if j not in rows_to_drop and (
                    dedup_across_vrd or df.iloc[i]["vrd"] == df.iloc[j]["vrd"]
                ):
                    ...
                    if (
                        scores.fmeasure > threshold["fmeasure"]
                        or scores.precision > threshold["precision"]
                        or scores.recall > threshold["recall"]
                    ):
    ...
    def deduplicate_embed(
        ...
        if isinstance(threshold, dict):
            threshold_value = threshold.get(
                "Value", self.model.config.system_params["embed_threshold"]["Value"]
            )
            threshold_right = threshold.get(
                "Right", self.model.config.system_params["embed_threshold"]["Right"]
            )
            threshold_duty = threshold.get(
                "Duty", self.model.config.system_params["embed_threshold"]["Duty"]
            )
            # check if there are other thresholds - throw error if so
            if not set(threshold.keys()).issubset({"Value", "Right", "Duty"}):
                raise ValueError(
                    "Threshold dictionary can only contain 'Value', 'Right', and 'Duty'"
                )
        else:
        ...
        for i in range(1, len(df)):
            for j in range(i):
                ...
                if j not in rows_to_drop and (
                    df.iloc[i]["vrd"] == df.iloc[j]["vrd"] or dedup_across_vrd
                ):
                    sim = self.cosine_similarity(
                        df.iloc[i]["embedding"], df.iloc[j]["embedding"]
                    )
                    if (
                        (df.iloc[i]["vrd"] == "Value" and sim > threshold_value)
                        or (df.iloc[i]["vrd"] == "Right" and sim > threshold_right)
                        or (df.iloc[i]["vrd"] == "Duty" and sim > threshold_duty)
                    ):
                        rows_to_drop.append(i)
                        break
    ...
    def clean_vrd(self, vrd):
        ...
        if "value" in vrd.lower():
            ...
            raise ValueError(
                'Invalid VRD - {}. Must be "Value", "Right", or "Duty"'.format(vrd)
            )

    def get_all_scores(
        self, actions, vrds, texts, explain=False, explanation_decoding_params={}
    ):
        ...
        # explain
        if explain:
            df["explanation"] = self.get_explanation(
                actions,
                vrds,
                texts,
                explanation_decoding_params=explanation_decoding_params,
            )
        elif explain:
    ...
    def get_output(
        ...
        df = self.get_all_scores(
            [action] * len(parsed),
            [d["vrd"] for d in parsed],
            [d["text"] for d in parsed],
            explain=explain,
            explanation_decoding_params=explanation_decoding_params,
        )
        # sort by relevance
    ...
    def get_candidates(
        self, action, explain=False, get_embeddings=True, explanation_decoding_params={}
    ):
        ...
        df = self.deduplicate(
            df, dedup_across_vrd=self.model.config.system_params["dedup_across_vrd"]
        )
    ...
    def get_explanation(
        self, actions, vrds, texts, batch_size=None, explanation_decoding_params={}
    ):
        ...
        for i in self.tqdm(range(n_batches), desc="Explanation"):
            ...
            encoded_batch = (
                self.tokenizer(
                    [
                        self.explanation_template(action, vrd, text)
                        for action, vrd, text in zip(
                            batch_actions, batch_vrds, batch_texts
                        )
                    ],
                    return_tensors="pt",
                    padding=True,
    ...
    def get_probs(self, inputs, batch_size=None):
        ...
        with torch.no_grad():
            ...
            for i in self.tqdm(range(n_batches + 1), desc="Inference"):
                ...
                inds = list(
                    range(i * batch_size, min((i + 1) * batch_size, len(inputs)))
                )
                ...
                logits_batch = (
                    self.model(
                        ...
                        labels=self.get_dummy(encoded_batch.input_ids),
                    )
                    .logits[:, -1, :]
                    .detach()
                    .cpu()
    ...
    def get_probs_template(
        self, actions, vrds, texts, template, token_ids, batch_size=None
    ):
        # clean vrds
        vrds = [self.clean_vrd(vrd) for vrd in vrds]
        # templatize
