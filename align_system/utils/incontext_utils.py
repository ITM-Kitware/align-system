...
from align_system.prompt_engineering.outlines_prompts import (
    ...
    probe_reasoning_comparative_kdma_score_prediction_json_schema,
...
class ProbeReasoningComparativeRegressionIncontextExampleGenerator(IncontextExampleGenerator):
    def set_icl_datasets(self):
        icl_datasets = {}
        incontext_data = self._read_icl_dataset_files()
        ...
        for target_kdma in self.target_kdmas:
            ...
            for example in kdma_incontext_data:
                ...
                for action, choice, kdma_value in zip(example['actions'], example['choices'], example["kdma_values"]):
                    ...
                    included_choices.append(choice)
                # Check if response is valid against json schema
                icl_response['reasoning'] = self.get_chain_of_thought_reasoning(target_kdma, 
                                                                                icl_response)
                correct_schema = json.loads(probe_reasoning_comparative_kdma_score_prediction_json_schema(included_choices, target_kdma["factor"]))
                validate(instance=icl_response, schema=correct_schema)
    ...
    def get_chain_of_thought_reasoning(self, target_kdma, choice_values):
        '''
        Helper function for set_icl_datasets() - constructs example reasoning statements for responses
        '''
        max_value = 0
        for choice, score in choice_values.items():
            if score['score'] >= max_value:
                max_value = score['score']
                high_choice = choice

        cot_reasoning = f'{high_choice} scores higher for {target_kdma['kdma']}.'
        return cot_reasoning